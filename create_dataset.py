...
def preprocess_dataset(
    ...
    for example in dataset:
        # Process text
        text = (
            f"<|TEXT_UNDERSTANDING_START|>{example['text']}<|TEXT_UNDERSTANDING_END|>"
        )
        ...
        with torch.no_grad():
            speech_codes = codec_model.encode_code(waveform.unsqueeze(0).cuda())[
                0, 0
            ]  # Get first codebook
        speech_ids = [
    ...
    # Save to memmap
    arr = np.memmap(
        memmap_path, dtype=np.int32, mode="w+", shape=(len(all_sequences), max_length)
    )
