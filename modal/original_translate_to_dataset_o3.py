...
if __name__ == "__main__":
    ...
    parser.add_argument('--cache_dir', type=str, default='/path/to/cache_folder', help='Directory for caching intermediate results')
    ...
    for split in ds.keys():
        ...
        ds_split = ds_split.map(add_length, batched=True, num_proc=4, batch_size=5000, input_columns=["start", "end"],
                                cache_file_name=f"{args.cache_dir}/audio_len_{split}.arrow")
        ...
        ds_split = ds_split.select(sorted_indices,
                                   indices_cache_file_name=f"{args.cache_dir}/sorting_indices_{split}.json")
        ...
        with torch.inference_mode(), torch.amp.autocast(device_type="cuda"):
            for batch in tqdm(dataloader, desc=f"Processing split {split}"):
                ...
                for i in range(batch_size):
                    ...
                    # Convert speech codes to list of ints.
                    speech_codes = speech_codes_tensor.cpu().tolist()
                    # Create speech token strings only once.
                    speech_token_strs = [f"<|s_{int(code)}|>" for code in speech_codes]
                    # Convert the speech token strings to IDs.
                    speech_ids = (
                        [SPEECH_GEN_START_ID]
                    ...
                    if MAX_TEXT_SPACE < 0:
                        continue  # or handle differently as needed

                    # Truncate text tokens if required.
